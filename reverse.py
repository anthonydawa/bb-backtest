...
SL_PERCENT_lONG = SL_PERCENT_SHORT * 1.2
CANDLE_COUNT_LONG = 0
...
header = "Timestamp,PNL,LongPNL,ShortPNL,balance,balancelong,balanceshort,entry,position"
logging.info(header)
...
for index, row in df.iterrows():
    ...
    log_line = f"{row['timestamp']},{pnl},{long_pnl},{short_pnl},{balance},{balance_long},{balance_short},{entry},{position}"
    # Log the line
    ...
    if candle_closed > 0:
        ...
        if position is None :
            ...
            # elif float(row['low']) <= float(row['LowerBand']) <= float(row['high']) and row['K'] <= 20:
                ...
                takeProfit = entry + (entry * SL_PERCENT_lONG)
