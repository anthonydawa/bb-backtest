...
def unix_millis_to_day_of_week(unix_millis):
    # Convert milliseconds to seconds
    unix_seconds = unix_millis / 1000

    # Get the day of the week as an integer (0 = Monday, 6 = Sunday)
    day_of_week_index = datetime.utcfromtimestamp(unix_seconds).weekday()

    # Map the integer to the day of the week string
...
bb_window = 17
SL_PERCENT_lONG = 0.7
SL_PERCENT_SHORT = 0.7
CANDLE_COUNT_LONG = 0.1
CANDLE_COUNT_SHORT = 0.1
...
for index, row in df.iterrows():
    ...
    elif unix_millis_to_day_of_week(row['timestamp']) == "Monday":
        pass
    elif candle_closed == 0 and unix_millis_to_day_of_week(row['timestamp']) == "Monday":
