...
bb_std_dev = 2.8
bb_window = 16
SL_PERCENT_lONG = 0.075
SL_PERCENT_SHORT = 0.225
CANDLE_COUNT_LONG = 0
CANDLE_COUNT_SHORT = 19
filename = f"backtest/TEST-{symbol}-{timeframe}-{start_date}-{end_date}-{bb_std_dev}-{bb_window}.csv"
# Fetch historical Klines
klines = client.futures_historical_klines_generator(symbol=symbol, interval=timeframe, start_str=start_date,end_str=end_date)
...
def calculate_ema(df,name,period=14):
    df[name] = df['close'].astype(float).ewm(span=period, adjust=False).mean()
...
calculate_ema(df,'EMA8',8)
calculate_ema(df,'EMA21',21)
    ...
    if candle_closed > 0:
        ...
        if position is None :
            ...
            if position == "Long":
                ...
                trend = "Long" if row['EMA8'] > row['EMA21'] else "Short"
            ...
            if position == "Long" and trend == "Short" and condition_1:
            ...
            if position == "Long" and trend == "Long" and not condition_1:
                    condition_1 = True 
                    continue

            if position == "Short" and trend == "Long" and condition_1:
                
                if entry < row["close"]:
                    percent_difference = abs(((row["close"] - entry) / entry) * 100)
                    net_percentage = percent_difference + fees
                    pnl -= net_percentage
                    short_pnl -= net_percentage
                    net_difference = abs((balance * ( net_percentage / 100) ))
            ...
            if position == "Short" and trend == "Short" and not condition_1:
                condition_1 = True
                continue
