...
for index, row in df.iterrows():
    ...
    if position is None and candle_closed == 0:
    ...
    elif position is not None and candle_closed == 0:
        ...
        elif position == "Short" and candle_closed == 0:
        ...
        if position == "Long" and candle_closed == 0:

            cut_loss_long = entry - ( entry * PERCENT_LIMIT )
            
            if float(row['low']) <= cut_loss_long <= float(row['high']):
                
                percent_difference = abs(((cut_loss_long - entry) / entry) * 100)
                position = None
                net_percentage = percent_difference + fees
                pnl -= net_percentage
                long_pnl -= net_percentage                   
            ...
            cut_loss_short = entry + ( entry * PERCENT_LIMIT )

            if float(row['low']) <= cut_loss_short <= float(row['high']):

                percent_difference = abs(((cut_loss_short - entry) / entry) * 100)
                position = None   
                net_percentage = percent_difference + fees
                pnl -= net_percentage
                short_pnl -= net_percentage
